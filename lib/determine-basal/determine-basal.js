...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    // if fewer than 25% of carbs have absorbed, adjust remainingCATime gradually from 3h up to 4h
    if ( meal_data.mealCOB > 0.75 * meal_data.carbs ) {
        // number of hours to adjust remainingCATime.  COB=carbs -> -1, COB=0.75*carbs -> 0
        fractionCOBAbsorbed = round(( meal_data.carbs - meal_data.mealCOB ) / meal_data.carbs,2);
        remainingCATimeAdjustment = Math.min(1,(fractionCOBAbsorbed*4))-1;
