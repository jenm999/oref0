...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    if (profile.temptargetSet && ( target_bg < 90 || target_bg > 110 )) {
        // conversely, assume 5h for 120, 6h for 140, etc.
        // number of hours to adjust remainingCATime.  80 -> -1, 120 -> +1, 140 -> +2
        remainingCATimeAdjustment = (target_bg - 100) / 20;
        remainingCATime += remainingCATimeAdjustment;
        console.error("Adjusting remainingCATime to",remainingCATime,"h based on temp target of",target_bg);
    // if fewer than 50% of carbs have absorbed, adjust remainingCATime gradually from 3h up to 4h
    } else { // if ( meal_data.mealCOB > 0.5 * meal_data.carbs ) {
        // number of hours to adjust remainingCATime.  COB=carbs -> -1, COB=0.5*carbs -> 0
        fractionCOBAbsorbed = round(( meal_data.carbs - meal_data.mealCOB ) / meal_data.carbs,2);
        remainingCATimeAdjustment = Math.min(1,(fractionCOBAbsorbed*2))-1;
        remainingCATime += remainingCATimeAdjustment;
        //console.error(fractionCOBAbsorbed, remainingCATimeAdjustment, remainingCATime)
        console.error("Adjusting remainingCATime to",remainingCATime,"h based on",fractionCOBAbsorbed*100+"% carb absorption");
    }
